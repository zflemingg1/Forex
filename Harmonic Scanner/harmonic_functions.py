...
def peak_detect(price,order_number):
	print repr(order_number) + " In Harmonic Function"
	max_index = list(argrelextrema(price, np.greater, order=order_number)[0])
	min_index = list(argrelextrema(price, np.less, order=order_number)[0])
	
	index = max_index + min_index + [len(price) - 1]
	index.sort()
	current_index = index[-5:] # get the last 5 peaks --> harmonics are 5 peaks in ratio to each other
	...
	return current_index,current_pattern,start,end
...
def is_bat(moves,err_allowed):
	...
	# Gartley Pattern Range
	AB_range = np.array([0.786 - err_allowed, 0.5 + err_allowed]) * abs(XA)
	BC_range = np.array([0.382 - err_allowed, 0.886 + err_allowed]) * abs(AB)
	CD_range = np.array([1.618 - err_allowed, 2.618 + err_allowed]) * abs(BC)
